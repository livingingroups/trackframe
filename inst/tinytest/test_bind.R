...
merge_tf1_tf2_all <- trackframe:::merge.trackframe(
  tf1,
  tf2,
  all = TRUE,
  sort = FALSE
...
expect_equal(
  merge_tf1_tf2_all,
  merge.data.frame(tf1, tf2, all = TRUE, sort = FALSE)[, c(
    "time",
    "id",
    "easting",
    "northing"
...
expect_equal(
  cbind_tf1_tf2,
  trackframe:::cbind.trackframe(tf1, as.data.frame(tf2))
)
...
## cbind ----
expect_error(
  trackframe:::cbind.trackframe(tf1, tf2d),
...
## rbind ----
expect_error(
  trackframe:::rbind.trackframe(tf1, tf2b, tf2c, tf2d),
  info = "crs of trackframes do not coincide"
)
expect_warning(trackframe:::rbind.trackframe(tf1, tf2, tf2c, tf2d))
expect_equal(
  trackframe:::rbind.trackframe(tf1, tf2, tf2c, tf2d),
...
## cbind ----
expect_error(
  trackframe:::cbind.trackframe(tf1, tf2b, tf2c, tf2d),
...
expect_equal(
  trackframe:::cbind.trackframe(tf1, tf2, tf2c, tf2b),
...
## cbind ----
expect_equal(
  trackframe:::cbind.trackframe(tf1, tf3),
...
## rbind ----
expect_error(
  trackframe:::rbind.trackframe(tf1, tf4),
...
## rbind ----
expect_error(
  trackframe:::rbind.trackframe(tf1, tf5),
  info = "numbers of columns of arguments do not match"
)
...
## cbind ----
expect_equal(
  trackframe:::cbind.trackframe(tf1, tf5),
...
tf8$time <- 1:11
expect_error(
  trackframe:::rbind.trackframe(tf1, tf8),
...
expect_error(
  trackframe:::rbind.trackframe(tf1, tf9),
  info = "numbers of columns of arguments do not match"
)
expect_error(
  trackframe:::merge.trackframe(tf1, tf9),
  info = "Not all tf_colnames(tf) are available in trackframe."
)
expect_equal(
  trackframe:::cbind.trackframe(tf1, tf9),
...
expect_error(
  trackframe:::merge.trackframe(tf1, df4),
  info = "Use by, or by.x and by.y explicitely when merging a trackframe with a non-trackframe
    data.frame/data.table/tibble"
)
expect_equal(
  trackframe:::merge.trackframe(tf1, df4, by = tf_colnames(tf1), sort = FALSE),
...
expect_equal(
  trackframe:::cbind.trackframe(tf1, data.table::as.data.table(dt4)),
