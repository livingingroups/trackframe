...
Trackframe only supports Cartesian (or Eucliean) coordinate systems, i.e., systems in which coordinates describe locations on a flat plane using perpendicular northing (y) and easting (x) axes, and units represent consistent physical distances in all directions.
...
If you don't already know the EPSG code for the UTM zone that your data are in, you can use trackframe's `suggest_utm_crs` function to find out. (Alternatively, or additionally, you can find or verify the UTM zone at <https://epsg.io/>).

```{r, eval=FALSE, include = FALSE}
# nolint start
# Get EPSG code for likely UTM zone
# suggest_utm_crs(albatross_move2)
