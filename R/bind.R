...
rbind.trackframe <- function(..., sort = FALSE){
  ...
  #check colnames
  stopifnot("Colnames are not equal for all objects." = Reduce(all.equal, lapply(objl, function(x) colnames(x))))
  # check time stamp format
  tcol <- time_col(objl[[1]])
  stopifnot("Class of time cols differ." = Reduce(all.equal, lapply(objl, function(x) class(x[[tcol]]))))
...
#' 
#' Merge two trackframes by key colums ("time", "id", "easting", "northing"), or do other versions of database join operations.
#'
...
#' tf2 <- tf1
#' tf2$id2 <- "A"
#' tf1_tf2 <- merge.trackframe(tf1, tf2)
#' tf1_tf2
#' class(tf1_tf2)
#' 
#' tf3 <- tf1
#' tf3$id <- c(rep("A",5), rep("B", 4), rep("C",2))
#' merge.trackframe(tf1, tf3, all = TRUE)
merge.trackframe <- function(x, y, sort = TRUE, suffixes = c("", ".y"), by = NULL, ...){
  ...
  if(any(!en_cols_y %in% by.y)) {
    if (isTRUE(all.equal(x[, tf_colnames(x), with = FALSE], y[, tf_colnames(y), with = FALSE], check.attributes = FALSE))) {
      y <- y[ , !colnames(y) %in% en_cols_y[!en_cols_y %in% by.y], with = FALSE]
    }
  } else {
    if (isTRUE(all.equal(x[, en_cols, with = FALSE], y[, en_cols_y, with = FALSE], check.attributes = FALSE))) {
  ...
  # mtf <- merge(x, y, by = by, sort = sort)
  # mtf <- merge(x, y, by = by, sort = sort, all =TRUE)
  # mtf <- merge(x, y, by = by, sort = sort, suffixes = c("", ".y"))
  mtf <- merge(x, y, by.x = by.x, by.y = by.y, sort = sort, suffixes = suffixes, ...)
  # mtf <- merge(x, y, by.x = by.x, by.y = by.y)
  # check for duplicates
